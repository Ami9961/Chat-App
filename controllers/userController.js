...
export const updateProfile = async(erq,res)=>{
    try{
        ...
        if(!profilePic){
            ...
            updatedUser = await User.findByAndUpdate(userId, {profilePic: upload.secure_url,bio,fullName},{new:true})
        }
        res.json({succes : true, user: up})
    } catch (error){
